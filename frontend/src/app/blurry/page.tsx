...
export default function BlurryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['blurry', page],
    queryFn:  () => mediaApi.blurry(page, 48).then(r => r.data),
  ...
  const trashMut = useMutation({
    ...
    onSuccess:  () => {
      qc.invalidateQueries({ queryKey: ['blurry'] });
    ...
    <div className="flex min-h-screen bg-neutral-950">
      ...
      <main className="flex-1 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl font-bold text-white">Blurry Photos</h1>
            <p className="text-sm text-neutral-400 mt-0.5">
              Detected by Laplacian variance analysis. Review and trash the ones you don't need.
            </p>
          </div>
          {data && data.total > 0 && (
            <span className="text-sm text-yellow-400 font-medium">
              {data.total} blurry photo{data.total !== 1 ? 's' : ''}
            </span>
          )}
        </div>

        {isLoading && <p className="text-neutral-500 text-sm">Analysing sharpness…</p>}

        {!isLoading && items.length === 0 && (
          <div className="flex flex-col items-center justify-center h-48 text-neutral-500 gap-2">
            <span className="text-3xl">🎯</span>
            <span className="text-sm">No blurry photos detected — great shooting!</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6
          ...
          {items.map((m, idx) => (
            <div key={m.id} className="relative group">
              ...
              {m.blurScore !== undefined && (
                <div className="absolute top-1 left-1/2 -translate-x-1/2 bg-black/70
                                text-yellow-400 text-[9px] px-1.5 rounded whitespace-nowrap">
                  score: {m.blurScore.toFixed(0)}
                </div>
              )}
              <button
