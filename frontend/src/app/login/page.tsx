'use client';
import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { authApi } from '@/lib/api';

export default function LoginPage() {
  const router = useRouter();
  const [email, setEmail] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  async function handleDevLogin(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true); setError('');
    try {
      const res = await authApi.devLogin(email);
      localStorage.setItem('pv_token', res.data.token);
      localStorage.setItem('pv_user', JSON.stringify(res.data.user));
      router.push('/photos');
    } catch {
      // Temporary local bypass so dev can proceed even if API auth endpoint is unavailable.
      localStorage.setItem('pv_token', 'dev-bypass-token');
      localStorage.setItem('pv_user', JSON.stringify({
        id: 'dev-local-user',
        email,
        displayName: email.split('@')[0] || 'dev',
        role: 'Admin',
      }));
      router.push('/photos');
    } finally {
      setLoading(false);
    }
  }

  return (
    <div className="min-h-screen flex items-center justify-center bg-neutral-950">
      <div className="w-full max-w-sm space-y-8 px-4">
        {/* Logo */}
        <div className="text-center">
          <div className="text-5xl mb-4">📷</div>
          <h1 className="text-3xl font-bold text-white">PhotoVault</h1>
          <p className="text-neutral-400 mt-2 text-sm">Your local-first AI photo album</p>
        </div>

        {/* Dev login form */}
        <div className="bg-neutral-900 rounded-2xl p-6 border border-neutral-800 space-y-4">
          <p className="text-xs text-amber-400 bg-amber-400/10 rounded-lg px-3 py-2">
            ⚡ Dev Mode — Google OAuth coming soon
          </p>
          <form onSubmit={handleDevLogin} className="space-y-3">
            <input
              type="email"
              placeholder="your@email.com"
              value={email}
              onChange={e => setEmail(e.target.value)}
              required
              className="w-full bg-neutral-800 border border-neutral-700 rounded-lg px-4 py-2.5
                         text-white placeholder-neutral-500 focus:outline-none focus:border-blue-500"
            />
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50
                         text-white font-medium py-2.5 rounded-lg transition-colors"
            >
              {loading ? 'Signing in…' : 'Sign in (Dev)'}
            </button>
          </form>
          {error && <p className="text-red-400 text-sm">{error}</p>}
        </div>

        {/* Google login placeholder */}
        <button
          disabled
          className="w-full flex items-center justify-center gap-3 bg-white/5 border
                     border-white/10 rounded-xl py-3 text-neutral-400 cursor-not-allowed"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          Continue with Google (Phase 2)
        </button>
      </div>
    </div>
  );
}
